...
export default function Home() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const newId = await addUser(formData);
      ...
      toast.error("Error saving user. Reverting changes...");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error("Error deleting user. Reverting changes...");
      // Revert optimistic update on error
      await fetchUsers();
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col min-h-screen p-8 gap-8 font-[family-name:var(--font-geist-sans)]">
      <h1 className="text-3xl font-bold text-center">Method 2: State Management with Optimistic Updates</h1>
      <p className="text-center text-gray-600 dark:text-gray-400">
        Changes are immediately reflected in UI while being sent to server
      </p>
